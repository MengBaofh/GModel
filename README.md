...
3.使用常规数据集划分方法（纵向或横向划分）时，Excel表中的点需要按照一定的顺序排列（例如：表中从序号为0开始，对应在图上的位置先从左到右，再从上到下）；  
4.使用自定义数据集划分方法时，可以直接导入训练集数据的索引（按列排布，即此处导入的Excel表的第一列为训练集索引，会自动进行训练集和验证集的划分）。  
